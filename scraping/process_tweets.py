...
def is_reply(s, user, reply_to):
    ...
    users_replied = [
        u for u in users if (u not in s) and (u.lower() != user.lower())
    ]
...
def clean_tweet_table(raw_data, length_threshold=10, likes_threshold=20):
    ...
    raw_data["tweet_cleaned"] = list(map(clean_text, raw_data.tweet))

    raw_data["is_reply"] = list(
        map(is_reply, raw_data.tweet, raw_data.username, raw_data.reply_to)
    )

    raw_data["is_short"] = list(
        map(
...
if __name__ == "__main__":
    ...
    for raw_file in raw_files:
        ...
        raw_data = pd.read_csv(raw_file, converters={"reply_to": literal_eval})
